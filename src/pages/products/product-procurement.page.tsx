        ...
        {loading || !product ? <div className="py-16 text-center text-sm text-slate-500"><Loader2 className="mx-auto mb-2 h-6 w-6 animate-spin" />Cargando expediente...</div> : (
          <div className="space-y-5">
            ...
            <section className="rounded-lg border border-slate-200 bg-white p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <h3 className="text-sm font-semibold text-slate-900">Ficha técnica para alta en Proscai</h3>
                  <p className="mt-1 text-xs text-slate-500">Datos capturados por Ventas que Compras puede validar y completar.</p>
                </div>
                <span className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${product.costStatus === "CONFIRMED" ? "bg-emerald-100 text-emerald-800" : product.costStatus === "ESTIMATED" ? "bg-blue-100 text-blue-800" : "bg-amber-100 text-amber-800"}`}>
                  {product.costStatus === "CONFIRMED" ? "Costo confirmado" : product.costStatus === "ESTIMATED" ? "Costo estimado" : "Costo pendiente"}
                </span>
              </div>
              <dl className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                <CatalogValue label="Familia" value={getLocalProductFamilyLabel(product.family)} />
                <CatalogValue label="Subfamilia" value={getLocalProductSubfamilyLabel(product.family, product.subfamily)} />
                <CatalogValue label="Marca" value={product.brand} />
                <CatalogValue label="Unidad" value={product.unit} />
              </dl>
              {product.commercialDescription && (
                <div className="mt-4 rounded-md bg-slate-50 p-3">
                  <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Descripción comercial</p>
                  <p className="mt-1 text-sm text-slate-700">{product.commercialDescription}</p>
                </div>
              )}
              {Object.keys(product.technicalAttributes || {}).length > 0 && (
                <dl className="mt-4 grid gap-3 border-t border-slate-100 pt-4 sm:grid-cols-2 lg:grid-cols-3">
                  {Object.entries(product.technicalAttributes).map(([key, value]) => (
                    <CatalogValue key={key} label={key.replaceAll("_", " ")} value={value} />
                  ))}
                </dl>
              )}
            </section>
...
const CatalogValue = ({ label, value }: { label: string; value: string | null }) => (
  <div>
    <dt className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">{label}</dt>
    <dd className="mt-1 text-sm font-semibold text-slate-700">{value || "Por definir"}</dd>
  </div>
);
