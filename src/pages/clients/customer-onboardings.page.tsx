...
import { AssignmentIndRounded, CheckCircleRounded, DeleteOutlineRounded, EditRounded, PictureAsPdfRounded, RefreshRounded, UploadFileRounded } from "@mui/icons-material";
import {
...
const deletableStatuses: CustomerOnboardingStatus[] = ["COLLECTING", "PENDING_REVIEW", "PENDING_CXC", "REJECTED", "READY_FOR_ERP", "COMPLETED", "CANCELLED"];
...
export const CustomerOnboardingsPanel = ({ focusOnboardingId }: { focusOnboardingId?: string | null }) => {
  ...
  const [deleteTarget, setDeleteTarget] = useState<CustomerOnboarding | null>(null);
  const [deleteConfirmation, setDeleteConfirmation] = useState("");
  const editable = Boolean(selected && (isAdmin || (isCxc
    ? selected.status === "PENDING_CXC"
    : ["COLLECTING", "PENDING_REVIEW", "REJECTED"].includes(selected.status))));
  ...
  useEffect(() => {
    if (!focusOnboardingId) return;
    let active = true;
    void CustomerOnboardingsService.get(focusOnboardingId).then((item) => {
      if (active) open(item);
    }).catch((error) => {
      if (active) notifier.error(error instanceof Error ? error.message : "No se pudo abrir el expediente.");
    });
    return () => { active = false; };
  }, [focusOnboardingId]);
  ...
  const approve = async () => {
    ...
    try {
      const updated = editable ? await CustomerOnboardingsService.update(selected.id, draft) : selected;
      setSelected(updated); setDraft(toInput(updated));
      setItems((current) => current.map((item) => item.id === updated.id ? updated : item));
      if (updated.missingFields.length) {
        notifier.warning("Se guardaron los cambios, pero faltan campos obligatorios antes de validar para ERP.");
        return;
      }
      const result = await CustomerOnboardingsService.approveForErp(updated.id);
      setSelected(result); setDraft(toInput(result));
      setItems((current) => current.map((item) => item.id === result.id ? result : item));
      notifier.success("Datos fiscales validados. El cliente está listo para registrarse en Proscai.");
    }
  ...
  const requestCorrection = async () => {
    if (!selected || !draft) return; setSaving(true);
    try {
      if (editable) await CustomerOnboardingsService.update(selected.id, draft);
      const result = await CustomerOnboardingsService.requestCorrection(selected.id, reviewReason);
      setSelected(result); setDraft(toInput(result)); setItems((current) => current.map((item) => item.id === result.id ? result : item));
  ...
  const confirmDelete = (item: CustomerOnboarding) => {
    setSelected(null);
    setDeleteConfirmation("");
    setDeleteTarget(item);
  };

  const deleteUnlinked = async () => {
    if (!deleteTarget || deleteConfirmation !== "ELIMINAR") return;
    setSaving(true);
    try {
      await CustomerOnboardingsService.remove(deleteTarget.id, deleteConfirmation);
      setItems((current) => current.filter((item) => item.id !== deleteTarget.id));
      setDeleteTarget(null);
      setDeleteConfirmation("");
      notifier.success("Alta fiscal eliminada. El cliente y sus cotizaciones se conservaron.");
    } catch (error) {
      notifier.error(error instanceof Error ? error.message : "No se pudo eliminar el alta fiscal.");
    } finally { setSaving(false); }
  };
  ...
  return (
    <Box sx={{ pt: 2.5, minHeight: "100%" }}>
      ...
      <Dialog open={Boolean(selected && draft)} onClose={() => !saving && setSelected(null)} fullWidth maxWidth="md">
        ...
        <DialogActions sx={{ px: 3, pb: 2.5, flexWrap: "wrap" }}><Button onClick={() => setSelected(null)} disabled={saving}>Cerrar</Button>{!isCxc && selected && ["COLLECTING", "PENDING_REVIEW", "REJECTED"].includes(selected.status) && <><Button variant="outlined" onClick={() => void save(false)} disabled={saving}>Guardar borrador</Button><Button variant="contained" color="warning" startIcon={saving ? <CircularProgress size={16} /> : <CheckCircleRounded />} onClick={() => void save(true)} disabled={saving || Boolean(selected.missingFields.length)}>Enviar a CxC</Button></>}{canReview && selected?.status === "PENDING_CXC" && <><Button variant="outlined" onClick={() => void save(false)} disabled={saving}>Guardar cambios</Button><Button color="warning" onClick={() => void requestCorrection()} disabled={saving || reviewReason.trim().length < 10}>Solicitar corrección</Button><Button variant="contained" color="success" onClick={() => void approve()} disabled={saving}>Validar para ERP</Button></>}{canReview && selected?.status === "READY_FOR_ERP" && <><TextField size="small" label="Código Proscai" value={erpCode} onChange={(event) => setErpCode(event.target.value)} /><Button variant="contained" color="success" onClick={() => void linkErp()} disabled={saving || !erpCode.trim()}>Marcar vinculado</Button></>}</DialogActions>
      </Dialog>
      <Dialog open={Boolean(deleteTarget)} onClose={() => !saving && setDeleteTarget(null)} fullWidth maxWidth="xs">
        <DialogTitle>Eliminar alta fiscal</DialogTitle>
        <DialogContent>
          <Typography variant="body2" sx={{ mb: 2 }}>
            Se eliminará el expediente fiscal de <strong>{deleteTarget?.legalName || deleteTarget?.customer.displayName}</strong> y la constancia subida directamente a él. El cliente, sus cotizaciones y los archivos originales del chat se conservarán.
          </Typography>
          <Alert severity="warning" sx={{ mb: 2 }}>
            No se puede deshacer. Si el alta ya estaba lista para ERP, el perfil fiscal del cliente volverá a Prospecto, pero conservará sus datos capturados. Un alta vinculada con Proscai no se puede eliminar.
          </Alert>
          <TextField fullWidth autoFocus label="Escribe ELIMINAR para confirmar" value={deleteConfirmation} onChange={(event) => setDeleteConfirmation(event.target.value)} disabled={saving} />
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2.5 }}>
          <Button onClick={() => setDeleteTarget(null)} disabled={saving}>Cancelar</Button>
          <Button variant="contained" color="error" onClick={() => void deleteUnlinked()} disabled={saving || deleteConfirmation !== "ELIMINAR"} startIcon={<DeleteOutlineRounded />}>Eliminar alta</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );
