...
const canPreview = (file: FileAttachment): boolean => FilePreviewService.kind(file) !== "unsupported";
      ...
      {files.map((file) => (
        <article key={file.id} className="flex flex-col gap-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex shrink-0 gap-1">
            {canPreview(file) && onPreview && <button type="button" onClick={() => onPreview(file)} disabled={busyFileId === file.id} className="rounded-md border border-amber-300 bg-amber-50 p-1.5 text-amber-800 hover:bg-amber-100 disabled:opacity-50" title={`Ver ${FilePreviewService.typeLabel(file)}`}><Eye className="h-3.5 w-3.5" /></button>}
